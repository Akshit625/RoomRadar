...
const { demoUsers, gehuProperties } = require("./gehuSeedData");

const seedGehu = async () => {
  await connectDB();
  ...
  await Property.create(
    gehuProperties.map((propertyData) => {
  ...
  console.log("GEHU property seed inserted.");
  process.exit(0);
};

seedGehu().catch((error) => {
  console.error("GEHU seeding failed:", error);
  process.exit(1);
});
